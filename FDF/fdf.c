...
/*   Updated: 2023/09/05 11:57:32 by xortega          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long int	ints_in_line(t_data *data, int reset)
{
	...
	{
		if (ft_isdigit(data->line[count]) == 1 && bolean == 0)
		{
			intnbr++;
			bolean = 1;
		}
...
void	fill_map(t_data *data)
{
	...
	while (lines >= 0)
	{
		...
		while (ints >= 0)
		{
			data->map[lines][ints] = i_atoi(data->line, spcs, data->line_s);
			if (data->color == 1)
				data->color_map[lines][ints] = \
				c_atoi(data->line, spcs, data->line_s);
			spcs += (data->big_int - f_size(data->map[lines][ints]) + 1);
			ft_printf("LINE: %d /INT : %d /INTS LEFT: %d\n", lines + 1, data->map[lines][ints], ints);
			ints--;
		}
